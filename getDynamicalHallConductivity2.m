...
% ----- transitions only between occ -> unocc
                ...
                for m = 1:dim
                    if ek(m) > ef
                        if abs(ek(n) - ek(m)) > 1e-2
                            hc = hc + imag(vop_x(n,m) * vop_y(m,n))/((ek(n)-ek(m))^2 - (freq+1i*smallDelta)^2);
                            shc = shc + imag(sop_x(n,m) * vop_y(m,n))/((ek(n)-ek(m))^2 - (freq+1i*smallDelta)^2);
                        end
                    end
                end
                dhc(kc, freqc, n) = hc;
